<div align="center">
  ...
  <h1>JavaScript Questions - VS LOGS</h1>
</div>
...
- A: `VSLOGS` and `undefined`
- B: `VSLOGS` and `ReferenceError`
...
The string `'VSLOGS'` is a truthy value. What we're actually asking, is "Is this truthy value falsy?". This returns `false`.
...
const member = new Person('VS', 'LOGS');
...
- A: `Person {firstName: "VS", lastName: "LOGS"}` and `undefined`
- B: `Person {firstName: "VS", lastName: "LOGS"}` and `Person {firstName: "Sarah", lastName: "Smith"}`
- C: `Person {firstName: "VS", lastName: "LOGS"}` and `{}`
- D: `Person {firstName: "VS", lastName: "LOGS"}` and `ReferenceError`
...
- A: `"VSLOGS"` `21` `["", " is ", " years old"]`
- B: `["", " is ", " years old"]` `"VSLOGS"` `21`
- C: `"VSLOGS"` `["", " is ", " years old"]` `21`
...
String.prototype.givePizza = () => {
  return 'Just give pizza already!';
};
...
- A: `undefined is 21` `VSLOGS is 21`
- B: `function` `function`
- C: `VSLOGS is 21` `VSLOGS is 21`
- D: `VSLOGS is 21` `function`
...
- A: `{ name: "VSLOGS" }, { age: 21 }`
- B: `"name", "age"`
- C: `"VSLOGS", 21`
- D: `["name", "VSLOGS"], ["age", 21]`
...
The value of `person` is an object. The argument `member` has a (copied) reference to the _same_ object. When we modify a property of the object `member` has a reference to, the value of `person` will also be modified, since they both have a reference to the same object. `person`'s `name` property is now equal to the value `"VSLOGS"`
...
```javascript
const user = { name: 'VSLOGS', age: 21 };
const admin = { admin: true, ...user };

console.log(admin);
```

- A: `{ admin: true, user: { name: "VSLOGS", age: 21 } }`
- B: `{ admin: true, name: "VSLOGS", age: 21 }`
- C: `{ admin: true, user: ["VSLOGS", 21] }`
- D: `{ admin: true }`
...
It's possible to combine objects using the spread operator `...`. It lets you create copies of the key/value pairs of one object, and add them to another object. In this case, we create copies of the `user` object, and add them to the `admin` object. The `admin` object now contains the copied key/value pairs, which results in `{ admin: true, name: "VSLOGS", age: 21 }`.
...
- A: `{ name: "VSLOGS", age: 21 }`, `["name", "age"]`
- B: `{ name: "VSLOGS", age: 21 }`, `["name"]`
- C: `{ name: "VSLOGS"}`, `["name", "age"]`
- D: `{ name: "VSLOGS"}`, `["age"]`
...
- B: `"{"username": "VSLOGS"}"`
- C: `"["level", "health"]"`
- D: `"{"username": "VSLOGS", "level":19, "health":90}"`
...
```javascript
const name = 'VSLOGS';
console.log(name.padStart(7));
console.log(name.padStart(2));
```

- A: `"VSLOGS"`, `"VSLOGS"`
- B: `" VSLOGS"`, `" VSLOGS"` (`"[13x whitespace]VSLOGS"`, `"[2x whitespace]VSLOGS"`)
- C: `" VSLOGS"`, `"VSLOGS"` (`"[1x whitespace]VSLOGS"`, `"VSLOGS"`)
- D: `"VSLOGS"`, `"OGS"`,
...
With the `padStart` method, we can add padding to the beginning of a string. The value passed to this method is the _total_ length of the string together with the padding. The string `"VSLOGS"` has a length of `6`. `name.padStart(7)` inserts 1 space at the start of the string, because 6 + 1 is 7.
...
```javascript
const { firstName: myName } = { firstName: 'VSLOGS' };
...
```javascript
const { firstName } = { firstName: 'VSLOGS' };
// ES5 version:
// var firstName = { firstName: 'VSLOGS' }.firstName;

console.log(firstName); // "VSLOGS"
```
...
const { firstName: myName } = { firstName: 'VSLOGS' };
// ES5 version:
// var myName = { firstName: 'VSLOGS' }.firstName;

console.log(myName); // "VSLOGS"
...
`function sayHi(name = "VSLOGS") { ... }`

In this case, if we didn't pass a value or if we passed `undefined`, `name` would always be equal to the string `VSLOGS`
...
- A: `{ name: "VSLOGS", age: 21 }`
- B: `{ name: "VSLOGS", age: 21, city: "Amsterdam" }`
- C: `{ name: "VSLOGS", age: 21, city: undefined }`
- D: `"Amsterdam"`
...
- A: `{ default: function default(), name: "VSLOGS" }`
- B: `{ default: function default() }`
- C: `{ default: "Hello world", name: "VSLOGS" }`
...
console.log(giveChocolate.prototype);
```
...
Regular functions, such as the `givePizza` function, have a `prototype` property, which is an object (prototype object) with a `constructor` property. Arrow functions however, such as the `giveChocolate` function, do not have this `prototype` property. `undefined` gets returned when trying to access the `prototype` property using `giveChocolate.prototype`.
...
- A: `name` `VSLOGS` and `age` `21`
- B: `["name", "VSLOGS"]` and `["age", 21]`
...
The first subarray is `[ "name", "VSLOGS" ]`, with `x` equal to `"name"`, and `y` equal to `"VSLOGS"`, which get logged.
...
- B: `[1, [2, 3, 4]]` and `{ name: "VSLOGS", age: 21 }`
- C: `[1, 2, 3, 4]` and `{ name: "VSLOGS", age: 21 }`
- D: `Error` and `{ name: "VSLOGS", age: 21 }`
...
- A: `3`, `NaN`, `NaN`
- B: `3`, `7`, `NaN`
- C: `3`, `VSLOGS2`, `[object Object]2`
- D: `"12"`, `VSLOGS2`, `[object Object]2`
...
However, the second one is a string `"VSLOGS"`. `"VSLOGS"` is a string and `2` is a number: `2` gets coerced into a string. `"VSLOGS"` and `"2"` get concatenated, which results in the string `"VSLOGS2"`.

`{ name: "VSLOGS" }` is an object. Neither a number nor an object is a string, so it stringifies both. Whenever we stringify a regular object, it becomes `"[object Object]"`. `"[object Object]"` concatenated with `"2"` becomes `"[object Object]2"`.
...
const jsonArray = JSON.stringify({ name: 'VSLOGS' }); // '{"name":"VSLOGS"}'
JSON.parse(jsonArray); // { name: 'VSLOGS' }
```
...
If we wouldn't have declared the `name` variable within the `getName` function, the javascript engine would've looked down the _scope chain_. The outer scope has a variable called `name` with the value of `VSLOGS`. In that case, it would've logged `VSLOGS`.
...
- A: `{name: "Sarah", age: 22}`
- B: `{name: "Sarah", age: 23}`
- C: `{name: "VSLOGS", age: 22}`
- D: `{name: "VSLOGS", age: 23}`
...
First, we invoke the `changeAge` function and pass the `person` object as its argument. This function increases the value of the `age` property by 1. `person` is now `{ name: "VSLOGS", age: 22 }`.

Then, we invoke the `changeAgeAndName` function, however we don't pass a parameter. Instead, the value of `x` is equal to a _new_ object: `{ ...person }`. Since it's a new object, it doesn't affect the values of the properties on the `person` object. `person` is still equal to `{ name: "VSLOGS", age: 22 }`.
...
- B: `Mara` `undefined` `VS LOGS` `ReferenceError`
- C: `Mara` `null` `VS LOGS` `null`
...
`person.getFullName?.()`: `person` has a property named `getFullName`: `person.getFullName()` is not nullish and can get invoked, which returns `VS LOGS`.
...
First, we invoke `funcOne`. On the first line of `funcOne`, we call the _asynchronous_ `setTimeout` function, from which the callback is sent to the Web API. (see my article on the event loop <a href="https://google.com">here</a>.)
...
First, we added a new property `name` to the proxy object (`person.name = "VSLOGS"`). `set` gets invoked, and logs `"Added a new property!"`.
...
###### 144. What do we need to add to the `person` object to get `["VS LOGS", 21]` as the output of `[...person]`?
...
Objects aren't iterable by default. An iterable is an iterable if the iterator protocol is present. We can add this manually by adding the iterator symbol `[Symbol.iterator]`, which has to return a generator object, for example by making it a generator function `*[Symbol.iterator]() {}`. This generator function has to yield the `Object.values` of the `person` object if we want it to return the array `["VS LOGS", 21]`: `yield* Object.values(this)`.
...
###### 153. What should the value of `method` be to log `{ name: "VSLOGS", age: 22 }`? 

```javascript
const keys = ["name", "age"]
const values = ["VSLOGS", 22]
...
This creates an array of subarrays containing the correct keys and values, which results in `{ name: "VSLOGS", age: 22 }`
